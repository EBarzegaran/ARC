...
if opt.redoANOAVfigs
    ...
    set(Fhandler,'PaperPositionMode','manual','PaperPosition',[.25 .25 2*numel(Orient) 6]);
    ...
    for Or = 1:numel(Orient)
        for i = 1:numel(PHStatResults)
            ...
            if strcmp(opt.Space,'Electrode')
                S = subplot(1,3,i);
                if noClust
                    ARC.Electrode_visaulization(PHStatResults{i}.Uncorrected.F',0,'parula',[]); axis tight
                else
                    ARC.Electrode_visualization(PHStatResults{i}.Uncorrected.F',0,'parula',SN); axis tight
                end
...
%                 %set(S,'position',SP+[-.05 -.05 -.05 -.05]);
                ...
                set(S,'position',get(S,'position')+[.03*(i-2) -.0 .0 .0]);
            ...
            elseif strcmp(opt.Space,'Source')
                S = subplot(numel(PHStatResults),numel(Orient),(i-1)*numel(Orient)+Or);
                load('MaptoSurface_400to5000.mat');
                Th =0.3;
                m1=PHStatResults{i}.Uncorrected.F*D;
                %m1=(m1-min(m1(:)))/(max(m1(:))-min(m1(:)));
                clusterm = zeros(size(PHStatResults{i}.Uncorrected.F));
                clusterm((SN)) = PHStatResults{i}.Uncorrected.F(SN);
                clusterm = clusterm*D;
    ...
    if noClust
        ...
        export_fig(Fhandler,fullfile(opt.ResultsPath,'GroupLevel',[FileName '_all'] ),'-pdf')
    end
    close;
                ...
                for cl = 1:numel(PHStatResults{i}.Clusters)
                    if ~isempty(PHStatResults{i}.Clusters(cl).PostHoc)
                        ...
                        for ph = 1:numel(PHresults)
                            ...
                            if strcmp(opt.Space,'Electrode')
                                ARC.Electrode_visualization(Nodes_F,0,'hotcortex',SN);axis tight
                            elseif strcmp(opt.Space,'Source')
                                load('MaptoSurface_400to5000.mat');
                                [~,MapInd] = (max(D'));
                                m1=Nodes_F'*D;
...
if opt.plotasd
    ...
    % why ASDchange is Nan, and orders in permuation test?
